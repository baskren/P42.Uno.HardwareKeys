...
{
    public static Action<Thread, Exception> DefaultExceptionHandler = Serilog.QuickLog.QLogExtensions.LogException;
    ...
    private static async Task ForgetAwaited(Task task, Action<Thread, Exception> onException, string callingMethodName = "")
    {
        try
        {
            await task;
        ...
        catch (Exception e)
        {
            ...
            Console.WriteLine($"Fire and forget task failed for calling method: {callingMethodName} [{e.Message}][{e.StackTrace}]");
